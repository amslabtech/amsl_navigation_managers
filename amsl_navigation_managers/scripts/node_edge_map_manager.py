...
from amsl_navigation_msgs.srv import (
    UpdateEdge,
    UpdateEdgeResponse,
    UpdateNode,
    UpdateNodeResponse,
...
class NodeEdgeMapManager:
    def __init__(self):
        ...
        self.node_marker_pub = rospy.Publisher(
            "/node_edge_map/viz/node", MarkerArray, queue_size=1, latch=True
        )
        self.edge_marker = Marker()
        self.edge_marker_pub = rospy.Publisher(
            "/node_edge_map/viz/edge", Marker, queue_size=1, latch=True
        )
        self.id_marker = MarkerArray()
        self.id_marker_pub = rospy.Publisher(
            "/node_edge_map/viz/id", MarkerArray, queue_size=1, latch=True
        )

        self.node_edge_map = NodeEdgeMap()
        self.node_edge_map_pub = rospy.Publisher(
            "/node_edge_map/map", NodeEdgeMap, queue_size=1, latch=True
        )

        self.update_node_server = rospy.Service(
            "/node_edge_map/update_node", UpdateNode, self.update_node_handler
        )
        self.update_edge_server = rospy.Service(
            "/node_edge_map/update_edge", UpdateEdge, self.update_edge_handler
        )
    ...
    def make_edge_marker(self):
        ...
        for edge in self.map_data["EDGE"]:
            p0 = Point()
            p0.x = self.map_data["NODE"][
                self.get_index_from_id(edge["node_id"][0])
            ]["point"]["x"]
            p0.y = self.map_data["NODE"][
                self.get_index_from_id(edge["node_id"][0])
            ]["point"]["y"]
            p1 = Point()
            p1.x = self.map_data["NODE"][
                self.get_index_from_id(edge["node_id"][1])
            ]["point"]["x"]
            p1.y = self.map_data["NODE"][
                self.get_index_from_id(edge["node_id"][1])
            ]["point"]["y"]
    ...
    def make_node_edge_map(self):
        ...
        for edge in self.map_data["EDGE"]:
            # forward
            e = Edge()
            x0 = self.map_data["NODE"][
                self.get_index_from_id(edge["node_id"][0])
            ]["point"]["x"]
            y0 = self.map_data["NODE"][
                self.get_index_from_id(edge["node_id"][0])
            ]["point"]["y"]
            x1 = self.map_data["NODE"][
                self.get_index_from_id(edge["node_id"][1])
            ]["point"]["x"]
            y1 = self.map_data["NODE"][
                self.get_index_from_id(edge["node_id"][1])
            ]["point"]["y"]
    ...
    def update_node_handler(self, request):
        ...
        if (
            ...
            or request.operation == request.MODIFY
        ):
            try:
                ...
                if index < 0:
                    # ADD
                    self.map_data["NODE"].append(
                        self.get_dict_from_node_msg(request.node)
                    )
                else:
                    # MODIFY
                    n = self.get_dict_from_node_msg(request.node)
    ...
    def get_dict_from_edge_msg(self, msg):
        edge_dict = {
            "node_id": [msg.node0_id, msg.node1_id],
            "impassable": msg.impassable,
        }
        return edge_dict
