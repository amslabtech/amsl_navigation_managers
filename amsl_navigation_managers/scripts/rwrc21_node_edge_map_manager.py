...
class NodeEdgeMapManager:
    def __init__(self):
        ...
        self.map_data = self.load_map_from_yaml() # yamlファイルの情報を受け取る(sample_map.yaml)
        ...
        self.node_edge_map_pub = rospy.Publisher('/node_edge_map/map', NodeEdgeMap, queue_size=1, latch=True) # 各node間の情報（距離、方位）
    ...
    def make_node_edge_map(self):
        ...
        for node in self.map_data['NODE']:
            ...
            self.node_edge_map.nodes.append(n) # node情報格納
        for edge in self.map_data['EDGE']: # 繋がりのあるnodeは(edge)すでにyamlで指定済み
            # forward
            e = Edge()
            ...
            e.distance = math.sqrt((x0 - x1) ** 2 + (y0 - y1) ** 2) # node間の距離計算
            e.direction = math.atan2((y1 - y0), (x1 - x0)); # node間の方位計算
