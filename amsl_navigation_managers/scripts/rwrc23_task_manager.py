...
class TaskManager:
    def __init__(self):
        ...
        self.cross_traffic_light_flag_sub = rospy.Subscriber(
            "/cross_traffic_light_flag", Bool, self.cross_traffic_light_flag_callback
        )
    ...
    def set_sound_volume(self):
        if self.enable_announce == True:
            volume_cmd = (
                "amixer -c1 sset Speaker " + str(self.sound_volume) + "%," + str(self.sound_volume) + "% unmute"
            )
    ...
    def use_dwa_planner(self):
        ...
        subprocess.Popen(
            ["rosrun", "topic_tools", "mux_select", str(self.finish_flag_topic), str(self.dwa_finish_flag)]
        )
        self.target_velocity.linear.x = self.dwa_target_velocity
    ...
    def use_point_follow_planner(self):
        ...
        subprocess.Popen(
            ["rosrun", "topic_tools", "mux_select", str(self.finish_flag_topic), str(self.pfp_finish_flag)]
        )
        self.target_velocity.linear.x = self.pfp_target_velocity
