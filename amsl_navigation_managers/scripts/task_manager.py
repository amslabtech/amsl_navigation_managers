...
class TaskManager:
    def __init__(self):
        ...
        self.pose_sub = rospy.Subscriber(
            "/estimated_pose/pose", Odometry, self.pose_callback
        )
        self.edge_sub = rospy.Subscriber(
            "/estimated_pose/edge", Edge, self.edge_callback
        )
        self.odom_sub = rospy.Subscriber(
            "/odom/complement", Odometry, self.odom_callback
        )
        ...
        self.local_goal_pub = rospy.Publisher(
            "/local_goal", PoseStamped, queue_size=1
        )
                ...
                if (
                    ...
                    if (
                        ...
                        if task["trigger"] == "enter/edge":
                            ...
                            task["trigger"]
                            == "recognition/stop_line/line_trace"
                        ):
                            if self.line_detected and not self.t_flag:
                                if self.line_info.center_point.x > 0.1:
                                    ...
                                    self.interrupt_local_goal(True)
                                    rospy.sleep(0.1)
                                    rel_local_goal = np.array(
                                        (
                                            self.line_info.center_point.x,
                                            self.line_info.center_point.y,
                                            0,
                                        )
                                    )
                                    self.publish_local_goal(
                                        rel_local_goal[:2],
                                        abs(self.line_info.angle),
                                    )
    ...
    def calc_line_direction(self, line_angle):
        robot_direction = self.get_yaw(
            self.estimated_pose.pose.pose.orientation
        )
    ...
    def calc_line_dist(self):
        ...
        dist = math.sqrt(
            (past_pose.x - curr_pose.x) ** 2 + (past_pose.y - curr_pose.y) ** 2
        )
        return dist
    ...
    def publish_local_goal(self, pose, line_angle):
        ...
        local_goal.pose.orientation = self.create_quaternion_from_yaw(
            line_direction
        )
