    ...
    def process(self):
        ...
        while not rospy.is_shutdown():
            ...
            for count, task in enumerate(self.task_data["task"]):
                # pprint(task)
                if (task["edge"]["node0_id"] == self.estimated_edge.node0_id) and (
                    task["edge"]["node1_id"] == self.estimated_edge.node1_id
                ):
                    # print("task ", count, " is related to this edge")
                    if (task["edge"]["progress_min"] < self.estimated_edge.progress) and (
                        self.estimated_edge.progress < task["edge"]["progress_max"]
                    ):
                                    ...
                                    if line_direction > math.pi * 0.25 and line_direction < math.pi * 0.75:
                                        ...
                                        if "performed" in task:
                                            ...
                                            abs_local_goal = np.array(
                                                (task["local_goal"]["x"], task["local_goal"]["y"], 0)
                                            )
                            ...
                            if self.line_detected and not self.t_flag:
                                if self.line_info.center_point.x > 0.1:
                                    ...
                                    rel_local_goal = np.array(
                                        (self.line_info.center_point.x, self.line_info.center_point.y, 0)
                                    )
