...
class MapMaker:
    def __init__(self, args):
        ...
        if args.INPUT_LATLNG_PATH is not None:
            self.make_map_from_latlng(
                args.INPUT_LATLNG_PATH, args.OUTPUT_MAP_PATH
            )
        ...
        if args.INPUT_MAP_PATH is not None:
            ...
            self.origin_utm = np.array(
                [
                    self.map_data["ORIGIN_UTM"]["x"],
                    self.map_data["ORIGIN_UTM"]["y"],
                ]
            )
...
class UTMUtils:
    ...
    def get_xy_from_latlng(self, lat, lng):
        ...
        t = np.sinh(
            np.arctanh(np.sin(phi_))
            - 2.0
            * np.sqrt(n)
            / (1 + n)
            * np.arctanh(2.0 * np.sqrt(n) / (1 + n) * np.sin(phi_))
        )
        ...
        E = E0 + k0 * A * (
            eta_
            + np.sum(alpha[1:] * np.cos(2 * j * xi_) * np.sinh(2 * j * eta_))
        )

        N = N0 + k0 * A * (
            xi_
            + np.sum(alpha[1:] * np.sin(2 * j * xi_) * np.cosh(2 * j * eta_))
        )
    ...
    def get_latlng_from_xy(self, x, y):
        ...
        eta_ = eta - np.sum(
            beta[1:] * np.cos(2 * j * xi) * np.sinh(2 * j * eta)
        )
    ...
    def get_alpha(self, n):
        alpha = np.array(
            [
                0,
                1.0 / 2.0 * n - 2.0 / 3.0 * n**2 + 5.0 / 16.0 * n**3,
    ...
    def get_beta(self, n):
        beta = np.array(
            [
                0,
                1.0 / 2.0 * n - 2.0 / 3.0 * n**2 + 37.0 / 96.0 * n**3,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--INPUT_MAP_PATH",
        type=str,
        help="map path to load (yaml)",
        default=None,
    )
    parser.add_argument(
        "--OUTPUT_MAP_PATH",
        type=str,
        help="map path to save (yaml)",
        default="test.yaml",
    )
    parser.add_argument(
        "--INPUT_LATLNG_PATH",
        type=str,
        help="input data path (csv)",
        default=None,
    )
